...
run_case() {
	...
	do
		...
		if [ $res -ne 1 ]; then
			...
			echo '<br>First day 1 fail<br>' >> ../mail_report							
			...
			if [ $debug_status = on ]; then
				echo '[DEBUG] First day 1 fail' >> ../$1
				echo '[DEBUG] First day 1 fail'
	...
	if [ $fail -ne 1 ]; then 	
		ssh $fe_ip_address "date 201201011101"
		...
		do
			...
			if [ $res -ne 1 ]; then
				...
				echo '<tr><td><b>Test '$2': </b><br>' >> ../mail_report
				echo '<br>case failed<br>' >> ../mail_report
				echo '<br>First day 9 fail<br>' >> ../mail_report							
				echo '<br>result = '$result'<br>' >> ../mail_report
				if [ $mail_logs_include = yes ]; then
					cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >> ../_mail_attaches/$2.log
				fi			
				echo '</td><td><font color="red"><b>Failed</b></font><br></td></tr>' >> ../mail_report			
				if [ $debug_status = on ]; then
					echo '[DEBUG] First day 9 fail' >> ../$1
					echo '[DEBUG] First day 9 fail'
					echo '[DEBUG] ipm_test result = '$result >> ../$1				
					echo '[DEBUG] ipm_test result = '$result								
				fi
				fail=1
			...
			if [ $res -eq 1 ]; then
				...
				do
					...
					if [ $res -ne 1 ]; then
						...
						echo '<tr><td><b>Test '$2': </b><br>' >> ../mail_report
						echo '<br>case failed<br>' >> ../mail_report
						echo '<br>second day 1 fail<br>' >> ../mail_report				
						echo '<br>result = '$result'<br>' >> ../mail_report
						if [ $mail_logs_include = yes ]; then
							cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >> ../_mail_attaches/$2.log
						fi			
						echo '</td><td><font color="red"><b>Failed</b></font><br></td></tr>' >> ../mail_report	
						if [ $debug_status = on ]; then
							echo '[DEBUG] second day 1 fail' >> ../$1
							echo '[DEBUG] second day 1 fail'
							echo '[DEBUG] ipm_test result = '$result >> ../$1				
							echo '[DEBUG] ipm_test result = '$result						
						fi				
						fail=1
				...
				if [ $fail -ne 1 ]; then 	
					ssh $fe_ip_address "date 201201021102"
					for i in 1 2 3 4 5 6 7 8 9
					do
						#result=`/usr/local/csn/utils/ipm/ipm_test --request ipm_request.xml --host $fe_ip_address:443`
						...
						if [ $res -ne 1 ]; then
							...
							echo '<tr><td><b>Test '$2': </b><br>' >> ../mail_report
							echo '<br>case failed<br>' >> ../mail_report
							echo '<br>second day 9 fail<br>' >> ../mail_report				
							echo '<br>result = '$result'<br>' >> ../mail_report
							if [ $mail_logs_include = yes ]; then
								cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >> ../_mail_attaches/$2.log
							fi			
							echo '</td><td><font color="red"><b>Failed</b></font><br></td></tr>' >> ../mail_report	
							if [ $debug_status = on ]; then
								echo '[DEBUG] second day 9 fail' >> ../$1
								echo '[DEBUG] second day 9 fail'
								echo '[DEBUG] ipm_test result = '$result >> ../$1				
								echo '[DEBUG] ipm_test result = '$result						
							fi				
							fail=1
					...
					if [ $fail -ne 1 ]; then 
						...
						if [ $res -eq 1 ]; then
							...
							echo '<tr><td><b>Test '$2': </b><br>' >> ../mail_report
							echo '<br>case failed<br>' >> ../mail_report
							echo '<br>second day 11 fail<br>' >> ../mail_report							
							echo '<br>result = '$result'<br>' >> ../mail_report
							if [ $mail_logs_include = yes ]; then
								cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >> ../_mail_attaches/$2.log
							fi			
							echo '</td><td><font color="red"><b>Failed</b></font><br></td></tr>' >> ../mail_report				
							if [ $debug_status = on ]; then
								echo '[DEBUG] second day 11 fail' >> ../$1
								echo '[DEBUG] second day 11 fail'
								echo '[DEBUG] ipm_test result = '$result >> ../$1				
								echo '[DEBUG] ipm_test result = '$result					
							fi			
							fail=1
