...
. ../testcase.ContentInfo.MaxDownloadCountPerDay.1_9-1_9/case.sh
...
run_restart $1 'testcase.ContentInfo.MaxDownloadCountPerDay.1_9-1_9'
...
if [ $restart_pass -eq 1 ]; then 
	run_case $1 'testcase.ContentInfo.MaxDownloadCountPerDay.1_9-1_9'
		if [ $debug_status = on ]; then
	...
	if [ $debug_status = on ]; then
		echo '[DEBUG] ContentInfo.MaxDownloadCountPerDay.1_9-1_9 : Fail' >> ../$1
	fi
	testsfail=`expr $testsfail + 1`
	errorexist=`expr $errorexist + 1`
	fail=1
	echo '<tr><td><b>Test testcase.ContentInfo.MaxDownloadCountPerDay.1_9-1_9: </b><br>' >> ../mail_report
	echo '<br>case failed<br>' >> ../mail_report
	echo '<br>restart frontend fail<br>' >> ../mail_report
	if [ $mail_logs_include = yes ]; then
		cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >>  ../_mail_attaches/testcase.ContentInfo.MaxDownloadCountPerDay.1_9-1_9.log
	fi			
