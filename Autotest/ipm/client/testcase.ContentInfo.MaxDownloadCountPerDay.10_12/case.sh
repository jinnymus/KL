...
run_case() {
	...
	do
		...
		if [ $res -ne 1 ]; then
			...
			echo '<br>First day 10 fail<br>' >> ../mail_report							
			...
			if [ $debug_status = on ]; then
				echo '[DEBUG] First day 10 fail' >> ../$1
				echo '[DEBUG] First day 10 fail'
	...
	if [ $fail -ne 1 ]; then 	
		...
		if [ $res -eq 1 ]; then
			if [ $debug_status = on ]; then
					echo '[DEBUG] first 10 day pass' >> ../$1
					echo '[DEBUG] first 10 day pass'
			fi		
			cp -rf content_info2.xml ../_db/content_info2.xml
			cp -rf geo_ip.xml ../_db/geo_ip2.xml
			mv -f ../_db/content_info2.xml ../_db/content_info.xml
			mv -f ../_db/geo_ip2.xml ../_db/geo_ip.xml
			sleep 3
			...
			do
				...
				if [ $res -ne 1 ]; then
					...
					echo '<tr><td><b>Test '$2': </b><br>' >> ../mail_report
					echo '<br>case failed<br>' >> ../mail_report
					echo '<br>first day 2 fail<br>' >> ../mail_report				
					echo '<br>result = '$result'<br>' >> ../mail_report
					if [ $mail_logs_include = yes ]; then
						cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >> ../_mail_attaches/$2.log
					fi			
					echo '</td><td><font color="red"><b>Failed</b></font><br></td></tr>' >> ../mail_report	
					if [ $debug_status = on ]; then
						echo '[DEBUG] first day 2 fail' >> ../$1
						echo '[DEBUG] first day 2 fail'
						echo '[DEBUG] ipm_test result = '$result >> ../$1				
						echo '[DEBUG] ipm_test result = '$result						
					fi				
					fail=1
			...
			if [ $fail -ne 1 ]; then 
				...
				if [ $res -eq 1 ]; then
					...
					echo '<tr><td><b>Test '$2': </b><br>' >> ../mail_report
					echo '<br>case failed<br>' >> ../mail_report
					echo '<br>first day 3 fail<br>' >> ../mail_report							
					echo '<br>result = '$result'<br>' >> ../mail_report
					if [ $mail_logs_include = yes ]; then
						cat ../_logs/$log_name | grep ContentDownloadCounter | tail -$log_tail_strings >> ../_mail_attaches/$2.log
					fi			
					echo '</td><td><font color="red"><b>Failed</b></font><br></td></tr>' >> ../mail_report				
					if [ $debug_status = on ]; then
						echo '[DEBUG] first day 3 fail' >> ../$1
						echo '[DEBUG] first day 3 fail'
						echo '[DEBUG] ipm_test result = '$result >> ../$1				
						echo '[DEBUG] ipm_test result = '$result					
					fi			
					fail=1
